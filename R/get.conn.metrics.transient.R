#' @title Get connectivity metrics for transient states
#'
...
#'
#' @examples
get.conn.metrics.transient <- function(suit.con.PA = inter.ep.in, nnet = net, shape.PA = all.ep) {
  ...
  add.suit.conn.area <- function(df) {
    if (sum(class(df) %in% c('data.table'))>0) {
      ...
      colnames(df)[colnames(df) == 'area.suit.t1'] <- "area.suit.t1.from.km2"
      df$area.suit.from.km2[is.na(df$area.suit.from.km2)] <- 0
      df$area.suit.t1.from.km2[is.na(df$area.suit.t1.from.km2)] <- 0
      
      df <- left_join(df, suit.con.PA, by = c('to' = 'SITECODE'))
      colnames(df)[colnames(df) == 'area.suit'] <- "area.suit.to.km2"
      colnames(df)[colnames(df) == 'area.suit.t1'] <- "area.suit.t1.to.km2"
      df$area.suit.to.km2[is.na(df$area.suit.to.km2)] <- 0
      df$area.suit.t1.to.km2[is.na(df$area.suit.t1.to.km2)] <- 0
    }
    return(df)
  }
  ...
  foreach::foreach(p=names(V(nnet))) %do% {  # DO NOT DO DOPAR OTHERWISE DOES NOT WORK 
    ...
    if (!is.null(short.p)) {
      short.p$pc <- short.p$area.suit.from.km2 * short.p$area.suit.t1.to.km2  * short.p$p_ij_shortP +
        short.p$area.suit.t1.from.km2 * short.p$area.suit.to.km2  * short.p$p_ij_shortP 
    ...
    PC_flux_i$PC_flux_i[PC_flux_i$SITECODE %in% pp] <- PC_flux_i$PC_flux_i[PC_flux_i$SITECODE %in% pp]  + pc.flux/(A*At1) 
  }
